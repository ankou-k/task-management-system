...
class DB_Utils():
    ...
    def create_project(session, user_id, name, description):
        ...
        print("hello from create project\n")

        # Create a Project object from the Python_Project instance
        ...
        recipient = session.query(User).filter_by(username=user_id).all()
        print(recipient)
    ...
    def delete_project(session, user_id, project_id):
        ...
        # get the tasks associated with that project
        project_tasks = session.query(Task).filter_by(project_id=project_id).all()
        # delete each task
        for task in project_tasks:
            # delete task assignments
            task_assignments = session.query(TaskAssignment).filter_by(task_id=task.id).all()
